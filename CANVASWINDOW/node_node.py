...
from GUIWINDOW.node_socket import Socket, LEFT_BOTTOM, LEFT_CENTER, LEFT_TOP, RIGHT_BOTTOM, RIGHT_CENTER, RIGHT_TOP
...
class Node(Serializable):
    ...
    def __init__(self, scene: 'Scene', title: str="Undefined Node", inputs: list=[], outputs: list=[]):
        ...
        :param scene: reference to the :class:`~GUIWINDOW.node_scene.Scene`
        :type scene: :class:`~nodeeditor.node_scene.Scene`
        :param title: Node Title shown in Scene
        :type title: str
        :param inputs: list of :class:`~GUIWINDOW.node_socket.Socket` types from which the `Sockets` will be auto created
        :param outputs: list of :class:`~GUIWINDOW.node_socket.Socket` types from which the `Sockets` will be auto created

        :Instance Attributes:

            - **scene** - reference to the :class:`~GUIWINDOW.node_scene.Scene`
            - **grNode** - Instance of :class:`~GUIWINDOW.node_graphics_node.QDMGraphicsNode` handling graphical representation in the ``QGraphicsScene``. Automatically created in the constructor
            - **content** - Instance of :class:`~GUIWINDOW.node_graphics_content.QDMGraphicsContent` which is child of ``QWidget`` representing container for all inner widgets inside of the Node. Automatically created in the constructor
            - **inputs** - list containin Input :class:`~GUIWINDOW.node_socket.Socket` instances
            - **outputs** - list containin Output :class:`~GUIWINDOW.node_socket.Socket` instances
    ...
    def getNodeContentClass(self):
        """Returns class representing GUIWINDOW content"""
    ...
    def onEdgeConnectionChanged(self, new_edge: 'Edge'):
        ...
        :param new_edge: reference to the changed :class:`~GUIWINDOW.node_edge.Edge`
    ...
    def onInputChanged(self, socket: 'Socket'):
        ...
        :param socket: reference to the changed :class:`~GUIWINDOW.node_socket.Socket`
    ...
    def hasConnectedEdge(self, edge: 'Edge'):
        """Returns ``True`` if edge is connected to any :class:`~GUIWINDOW.node_socket.Socket` of this `Node`"""
        for socket in (self.inputs + self.outputs):
...
#        if self.
    ...
    def getChildrenNodes(self) -> 'List[Node]':
        ...
        :rtype: List[:class:`~GUIWINDOW.node_node.Node`]
        """
    ...
    def getInput(self, index: int=0) -> ['Node', None]:
        ...
        :type index: ``int``
        :return: :class:`~GUIWINDOW.node_node.Node` which is connected to the specified `Input` or ``None`` if
    ...
    def getInputWithSocket(self, index: int=0) -> [('Node', 'Socket'), (None, None)]:
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: Tuple containing :class:`~GUIWINDOW.node_node.Node` and :class:`~GUIWINDOW.node_socket.Socket` which
    ...
    def getInputs(self, index: int=0) -> 'List[Node]':
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: all :class:`~GUIWINDOW.node_node.Node` instances which are connected to the
