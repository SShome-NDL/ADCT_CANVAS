...
from node.node_graphics_node import QDMGraphicsNode
...
class NodeEditorWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.name_product = 'ADCT GUI'

        self.initUI()


    def initUI(self):
        """Set up this ``QMainWindow``. Create :class:`~GUIWINDOW.node_editor_widget.NodeEditorWidget`, Actions and Menus"""
        self.createActions()
        self.createMenus()
    ...
    def isModified(self) -> bool:
        """Has current :class:`~GUIWINDOW.node_scene.Scene` been modified?

        :return: ``True`` if current :class:`~GUIWINDOW.node_scene.Scene` has been modified
        :rtype: ``bool``
        """
    ...
    def getCurrentNodeEditorWidget(self) -> NodeEditorWidget:
        """get current :class:`~GUIWINDOW.node_editor_widget`

        :return: get current :class:`~GUIWINDOW.node_editor_widget`
    ...
    def onEditCopy(self):
        ...
        if self.getCurrentNodeEditorWidget():
            print()
            print("282 node_editor_window", self.getCurrentNodeEditorWidget())
            print("285 node_editor_window",type(self.getCurrentNodeEditorWidget()))
            #print(QDMGraphicsNode.)
            data = copy.deepcopy(self.getCurrentNodeEditorWidget().scene.clipboard.serializeSelected(delete=False))
            from INTERNAL_SCENE.calc_sub_window import variableManager
            f_name_holder = copy.deepcopy(variableManager.f_name_holder)
            script_data = f_name_holder
            str_data = json.dumps(data, indent=4)
            QApplication.instance().clipboard().setText(str_data)
            #print(str_data)
            print(type(data))
            ...
            def collect_op_codes(d, op_codes):
                if isinstance(d, OrderedDict):
                    if 'op_code' in d:
                        op_codes.append(d['op_code'])
                    for value in d.values():
                        if isinstance(value, (OrderedDict, list)):
                            collect_op_codes(value, op_codes)
                elif isinstance(d, list):
                    for item in d:
                        collect_op_codes(item, op_codes)

            def get_content_by_id(d, target_id):
                if isinstance(d, OrderedDict):
                    if d.get('id') == target_id:
                        return d.get('content')
                    for value in d.values():
                        if isinstance(value, (OrderedDict, list)):
                            result = get_content_by_id(value, target_id)
                            if result is not None:  # If content is found, return it
                                return result
                elif isinstance(d, list):
                    for item in d:
                        result = get_content_by_id(item, target_id)
                        if result is not None:  # If content is found, return it
                            return result
                return fhgfh

            # Example usage to check if 'title' is 'lookUp'
            print(collect_op_codes(data, v))
            if 'title' in data: pass
            print("329 node_editor_window f_name-holder",data)
            node_ids = [node['id'] for node in data['nodes']]

            n_id=int(node_ids[0])
            n_id_deep = copy.deepcopy(n_id)
            print("335 node_editor_window deepcopy", n_id_deep)
            print("336 node_editor_window node id list",n_id)
            if n_id in variable.storage:
                print(f"Filename: {variable.storage[n_id]['filename']}")
                print(f"Delimiter: {variable.storage[n_id]['delimiter']}")

            v.reverse()
            print("op_codes =", v)

            for key, value in list(variable.storage.items()):
                new_obj = copy.deepcopy(value)
                new_key = id(new_obj)
                variable.storage[new_key] = new_obj
            print("348 updated dict on copy node_editor_window",variable.storage)

    def onEditPaste(self):
        """Handle Edit Paste from clipboard operation"""
        print("yes this one haha")
        if self.getCurrentNodeEditorWidget():
            raw_data = copy.deepcopy(QApplication.instance().clipboard().text())
            #print("347 node_editor_window",raw_data)
            try:
                data = copy.deepcopy(json.loads(raw_data))
