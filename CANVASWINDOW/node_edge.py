...
class Edge(Serializable):
    ...
    def start_socket(self, value):
        ...
        if self._start_socket is not None:
            ...
            print("edge ID", id(self.grEdge))
            print("Start Socket ID",id(self._start_socket))
            variable.edge_dict[id(self.grEdge)] = {"Start Socket":id(self._start_socket)}
            #print("Edge Dict",variable.edge_dict)
    ...
    def end_socket(self, value):
        ...
        if self._end_socket is not None:
            print("yes 80 node_edge")
            self._end_socket.removeEdge(self)
            print("end socket", id(self._end_socket))
            # variable.edge_dict[id(self.grEdge)]["End Socket"] = id(self._end_socket)
            # print("Edge Dict", variable.edge_dict)

        # assign new end socket
        self._end_socket= value
        print("yes 87 end_socket", id(value))
        # variable.edge_dict[id(self.grEdge)]["End Socket"] = id(value)
        # addEdge to the Socket class
        if self.end_socket is not None:
            print("yes 90 node_edge",id(self._end_socket))
    ...
    def serialize(self) -> OrderedDict:
        variable.edge_dict[id(self)] = {"Start Socket": id(self._start_socket),
                                        "End Socket": id(self._end_socket)}
        variable.edge_dict = {key:value for key, value in variable.edge_dict.items() if key in variable.edge_id}
        order_var_edge_dict = {}
        for key,value in variable.edge_dict.items():
            start_socket = value["Start Socket"]
            if start_socket not in order_var_edge_dict:
                order_var_edge_dict[start_socket] = {}
            order_var_edge_dict[start_socket][key] = value
        recorded_var_edge_dict = {}
        for start_socket,entries in order_var_edge_dict.items():
            recorded_var_edge_dict.update(entries)
        print("234 node_edge ##########################################Seriaized edge dict",variable.edge_dict)
        print("235 node_edge",recorded_var_edge_dict)
        return OrderedDict([
            ('id', self.id),
            ('edge_id', self.id),
